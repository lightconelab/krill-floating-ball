...
`月卡单窗口` 统计最早生效月卡的当前额度窗口；`月卡` 统计同一张月卡从生效开始到当前时间的累计使用情况。

| 月卡单窗口 | 月卡 |
| --- | --- |
| <img src="docs/images/stats-month-window.png" width="640" alt="月卡单窗口统计范围"> | <img src="docs/images/stats-month-plan.png" width="640" alt="月卡统计范围"> |
...
`月卡单窗口` 和 `月卡` 仅在存在生效月卡时可用，并以生效月卡中 `subscription_start_at` 最早的那张月卡为准。`月卡单窗口` 统计该月卡当前额度窗口从开始到现在的用量，`月卡` 统计该月卡从生效开始到现在的用量。
