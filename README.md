...
Krill Floating Ball 是一个原生 macOS 桌面悬浮工具，用于快速查看 Krill AI 的套餐额度、使用统计、缓存率、钱包余额和 Codex 模型智商排行。它以 80px 液体悬浮球展示当前额度水位，贴近屏幕边缘时可自动切换为细长贴边进度条，鼠标悬浮后展开完整信息面板。
...
<p align="center">
  <img src="docs/images/codex-model-iq.png" width="760" alt="Codex 模型智商排行">
</p>
...
- 鼠标悬浮展示展开栏，包含使用统计、钱包余额、刷新状态和所有生效套餐；套餐卡片每页展示 2 个，可通过标题栏分页浏览。
- 使用统计支持 `月卡单窗口`、`月卡`、`今日`、`7日`、`30日`。
- 花费、请求数和 Tokens 展示趋势折线；缓存率按渠道展示进度条。
- 展开栏可展示 Codex 模型智商横向排行，按数值从高到低排序。
...
- 展开栏展示使用统计、Codex 模型智商、缓存率、钱包余额、刷新状态和当前生效套餐。
- 生效套餐卡片展示套餐名称、时间范围、额度进度和剩余时间；超过 2 个套餐时按接口顺序分页展示。
