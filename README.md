...
| 菜单栏图标 | 菜单栏功能 | 登录弹窗 | 未配置账号 |
| --- | --- | --- | --- |
| <img src="docs/images/menubar-icon.png" width="96" alt="菜单栏图标"> | <img src="docs/images/menubar-menu.png" width="220" alt="菜单栏功能"> | <img src="docs/images/login-prompt.png" width="360" alt="Krill 登录弹窗"> | <img src="docs/images/missing-login-overview.png" width="520" alt="未配置账号状态"> |
...
- 支持手动刷新、开机启动、设置 Krill 登录账号、清除登录信息、隐藏/显示悬浮球和退出应用。
...
套餐卡片按套餐总额度的剩余额度从高到低排序。

### 套餐额度

- `plan.billing_type = usd_monthly`：总额度为 `quota.daily_limit_usd`，不单独计算周额度。
- `plan.billing_type = usd_weekly`：周额度为 `quota.daily_limit_usd`，总额度为 `quota.daily_limit_usd * 4`。
- 其他套餐类型会尽量按接口返回的额度字段做兼容展示。

### 悬浮球水位

悬浮球展示的是当前周周期可用池，而不是单个套餐额度：

```text
当前周周期可用池 = 生效中的周额度套餐 + 与当前周周期时间范围重叠的其他生效总额度套餐
```
...
4. 首次启动时输入 Krill AI 邮箱和密码，或从菜单栏选择 `设置 Krill 账号...`。
...
5. 在菜单栏中手动刷新、调整自动刷新间隔、开启或关闭开机启动、开启或关闭贴边进度条、清除登录信息或退出应用。
