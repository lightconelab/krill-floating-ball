...
- Renamed the usage statistic range buttons to `月卡单窗口` and `月卡`, with more stable monthly-plan context selection.
- Reduced refresh and statistic-switch memory overhead, especially for large statistic ranges, and released temporary account-dialog resources after close.
...
- Refreshed Chinese and English README screenshots for the expanded panel, floating overview, edge progress bar, menu, CPU, memory, and energy references.

## v0.2.7 - 2026-06-29

- Improved Krill login compatibility while keeping the app lightweight and avoiding browser engines.
...
- Changed active subscription cards to preserve the service-provided display order after inactive items are filtered out.
- Refreshed Chinese and English README screenshots and updated the documented menu labels, account flow, balance alert wording, and subscription ordering behavior.
- Simplified recently touched code paths around empty-state labels, network status handling, stat-card drawing, and unused widget callbacks while preserving behavior.

## v0.2.6 - 2026-06-25

- Switched the public setup flow to Krill email/password login stored in macOS Keychain, with session credentials kept in memory only.
...
- Tightened network memory behavior and released heap pressure after refresh and panel teardown.
- Rewrote the Chinese and English README for public open-source use, refreshed screenshots, and removed obsolete setup images.
...
- Reduced memory growth when switching larger statistic ranges by processing large ranges in smaller batches, limiting trend samples, and releasing heap pressure between batches.
...
- Added explicit request timeouts for lower network memory overhead.
- Reduced memory growth when switching statistic ranges by canceling stale refreshes, sampling trend data during parsing, and clearing hidden panel snapshots.
- Refreshed README screenshots for the expanded panel, floating overview, and edge-bar overview.
...
- Cached Keychain credential lookup after first successful load to reduce repeated password prompts in normal use.
- Updated active subscription display details, compact time labels, and quota rendering.
...
- Corrected the floating ball availability display across overlapping active items.
- Kept the liquid color behavior aligned with the displayed availability level.
- Fixed the missing-account state so the floating ball no longer renders a gray liquid wave before an account is configured.
- Sorted active subscription cards by remaining display value with expiry time as the tie-breaker.
- Refreshed README screenshots for the floating overview, expanded panel, floating ball, missing-account state, and account prompt.
- Updated Chinese and English documentation to describe the current display behavior.

## v0.2.1 - 2026-06-18

- Added refreshed README screenshots for the menu bar icon, menu actions, floating ball, expanded panel, and floating overview.
...
- Refined the floating ball liquid animation and alert color behavior while keeping low idle overhead.
...
- Corrected quota display for monthly and weekly plan types.
...
- Added an 80px liquid quota indicator.
...
- Added macOS Keychain storage for Krill credentials.
